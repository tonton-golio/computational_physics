...
function LinearRegressionDemo() {
  ...
  useEffect(() => {
    ...
    const y = x.map(xi => beta0 + beta1 * xi + math.random('normal', 0, noise));
    const meanX = math.mean(x);
    const meanY = math.mean(y);
    const covXY = math.mean(x.map((xi, i) => (xi - meanX) * (y[i] - meanY)));
    const varX = math.var(x);
    const b1 = covXY / varX;
    const b0 = meanY - b1 * meanX;
    const yHat = x.map(xi => b0 + b1 * xi);
    const residuals = y.map((yi, i) => yi - yHat[i]);
    const ssRes = math.sum(residuals.map(e => e * e));
    const sigmaHat = Math.sqrt(ssRes / (sampleSize - 2));
    const sumSqX = math.sum(x.map(xi => (xi - meanX) ** 2));
    ...
    <Card>
      ...
      <CardContent>
        ...
        <Plotly
          data={plotData}
          layout={{
            title: 'Linear Regression: Data, Fit, Residuals, and CI',
            xaxis: { title: 'x' },
            yaxis: { title: 'y' },
            height: 500
          }}
