...
function card(title: string, children: React.ReactNode): React.ReactElement {
  return (
    <div className="w-full rounded-lg bg-[#151525] p-6">
...
export function LossFunctionsDemo(): React.ReactElement {
  ...
  return card(
    ...
    <>
      <div className="mb-4">
        <label className="text-sm text-gray-300">Hinge margin: {margin.toFixed(2)}</label>
        <input
          type="range"
          min={0.2}
          max={2}
      ...
      <Plotly
        ...
        layout={{
          ...
          font: { color: '#9ca3af' },
          xaxis: { title: { text: 'Score/Residual' }, gridcolor: '#1e1e2e' },
  ...
  return card(
    ...
    <>
      <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          <label className="text-sm text-gray-300">Noise: {noise.toFixed(2)}</label>
          <input
            type="range"
            min={0}
            max={0.8}
        ...
        <div>
          <label className="text-sm text-gray-300">Ripple frequency: {ripple.toFixed(1)}</label>
          <input
            type="range"
            min={0.5}
            max={5}
...
export function ValidationSplitDemo(): React.ReactElement {
  ...
  return card(
    ...
    <>
      <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          <label className="text-sm text-gray-300">Epochs: {epochs}</label>
          <input
            type="range"
            min={15}
            max={80}
        ...
        <div>
          <label className="text-sm text-gray-300">Overfitting onset: {overfitStart}</label>
          <input
            type="range"
            min={5}
            max={60}
            step={1}
            value={overfitStart}
            onChange={(e) => setOverfitStart(Number(e.target.value))}
            className="w-full"
          />
        </div>
      </div>
      <Plotly
        ...
        layout={{
          ...
          plot_bgcolor: 'rgba(15,15,25,1)',
          font: { color: '#9ca3af' },
          xaxis: { title: { text: 'Epoch' }, gridcolor: '#1e1e2e' },
          yaxis: { title: { text: 'Loss' }, gridcolor: '#1e1e2e' },
          margin: { t: 20, r: 20, b: 45, l: 55 },
        }}
        config={{ displayModeBar: false }}
...
export function TreeSplitImpurityDemo(): React.ReactElement {
  ...
  return card(
    ...
    <>
      <div className="mb-4">
        <label className="text-sm text-gray-300">Class-1 probability: {p.toFixed(2)}</label>
        <input
          type="range"
          min={0.01}
          max={0.99}
      ...
      <Plotly
        ...
        layout={{
          ...
          plot_bgcolor: 'rgba(15,15,25,1)',
          font: { color: '#9ca3af' },
          xaxis: { title: { text: 'p(class 1)' }, gridcolor: '#1e1e2e' },
          yaxis: { title: { text: 'Impurity' }, gridcolor: '#1e1e2e' },
          margin: { t: 20, r: 20, b: 45, l: 55 },
        }}
        config={{ displayModeBar: false }}
        style={{ width: '100%', height: 360 }}
  ...
  return card(
    ...
    <>
      <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-3">
        <div>
          <label className="text-sm text-gray-300">Samples: {n}</label>
          <input type="range" min={100} max={1000} step={50} value={n} onChange={(e) => setN(Number(e.target.value))} className="w-full" />
        </div>
        <div>
          <label className="text-sm text-gray-300">Noise: {noise.toFixed(2)}</label>
          <input type="range" min={0.05} max={0.45} step={0.01} value={noise} onChange={(e) => setNoise(Number(e.target.value))} className="w-full" />
        </div>
        <div>
          <label className="text-sm text-gray-300">Decision threshold: {threshold.toFixed(2)}</label>
          <input type="range" min={0.3} max={0.8} step={0.01} value={threshold} onChange={(e) => setThreshold(Number(e.target.value))} className="w-full" />
        </div>
      </div>
      <Plotly
        data={[
...
export function PcaCorrelatedDataDemo(): React.ReactElement {
  ...
  return card(
    ...
    <>
      <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          <label className="text-sm text-gray-300">Correlation: {corr.toFixed(2)}</label>
          <input type="range" min={0.1} max={0.98} step={0.01} value={corr} onChange={(e) => setCorr(Number(e.target.value))} className="w-full" />
        </div>
        <div>
          <label className="text-sm text-gray-300">Samples: {n}</label>
          <input type="range" min={100} max={600} step={20} value={n} onChange={(e) => setN(Number(e.target.value))} className="w-full" />
        </div>
      </div>
      <Plotly
        data={[
        ...
        layout={{
          ...
          plot_bgcolor: 'rgba(15,15,25,1)',
          font: { color: '#9ca3af' },
          xaxis: { title: { text: 'x1' }, gridcolor: '#1e1e2e' },
          yaxis: { title: { text: 'x2' }, gridcolor: '#1e1e2e' },
          margin: { t: 20, r: 20, b: 45, l: 55 },
          annotations: [
            {
              xref: 'paper',
...
export function ExplainedVarianceDemo(): React.ReactElement {
  ...
  return card(
    ...
    <>
      <div className="mb-4">
        <label className="text-sm text-gray-300">Dimensions: {dims}</label>
        <input type="range" min={3} max={20} step={1} value={dims} onChange={(e) => setDims(Number(e.target.value))} className="w-full" />
      </div>
      <Plotly
        data={[
        ...
        layout={{
          ...
          plot_bgcolor: 'rgba(15,15,25,1)',
          font: { color: '#9ca3af' },
          xaxis: { title: { text: 'Component' }, gridcolor: '#1e1e2e' },
          yaxis: { title: { text: 'Explained variance ratio' }, gridcolor: '#1e1e2e' },
...
export function TsneUmapComparisonDemo(): React.ReactElement {
  ...
  return card(
    ...
    <>
      <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          <label className="text-sm text-gray-300">Cluster count: {clusters}</label>
          <input type="range" min={2} max={6} step={1} value={clusters} onChange={(e) => setClusters(Number(e.target.value))} className="w-full" />
        </div>
        <div>
          <label className="text-sm text-gray-300">Spread: {spread.toFixed(2)}</label>
          <input type="range" min={0.15} max={1.2} step={0.05} value={spread} onChange={(e) => setSpread(Number(e.target.value))} className="w-full" />
        </div>
      </div>
      <Plotly
        data={[
        ...
        layout={{
          ...
          showlegend: false,
          annotations: [
            { x: 0.13, y: 1.08, xref: 'paper', yref: 'paper', text: 'PCA', showarrow: false, font: { color: '#9ca3af' } },
            { x: 0.50, y: 1.08, xref: 'paper', yref: 'paper', text: 't-SNE', showarrow: false, font: { color: '#9ca3af' } },
            { x: 0.87, y: 1.08, xref: 'paper', yref: 'paper', text: 'UMAP', showarrow: false, font: { color: '#9ca3af' } },
          ],
        }}
        config={{ displayModeBar: false }}
        style={{ width: '100%', height: 360 }}
...
export function GraphConvolutionIntuitionDemo(): React.ReactElement {
  ...
  return card(
    ...
    <>
      <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          <label className="text-sm text-gray-300">Nodes: {n}</label>
          <input type="range" min={6} max={24} step={1} value={n} onChange={(e) => setN(Number(e.target.value))} className="w-full" />
        </div>
        <div>
          <label className="text-sm text-gray-300">Neighborhood radius: {radius}</label>
          <input type="range" min={1} max={4} step={1} value={radius} onChange={(e) => setRadius(Number(e.target.value))} className="w-full" />
        </div>
      </div>
      <Plotly
        ...
        layout={{
          ...
          plot_bgcolor: 'rgba(15,15,25,1)',
          font: { color: '#9ca3af' },
          xaxis: { title: { text: 'Node index' }, gridcolor: '#1e1e2e' },
          yaxis: { title: { text: 'Node index' }, gridcolor: '#1e1e2e' },
          margin: { t: 20, r: 20, b: 45, l: 55 },
        }}
        config={{ displayModeBar: false }}
        style={{ width: '100%', height: 360 }}
  ...
  return card(
    ...
    <>
      <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          <label className="text-sm text-gray-300">Nodes: {n}</label>
          <input type="range" min={6} max={25} step={1} value={n} onChange={(e) => setN(Number(e.target.value))} className="w-full" />
        </div>
        <div>
          <label className="text-sm text-gray-300">Edge probability: {p.toFixed(2)}</label>
          <input type="range" min={0.05} max={0.7} step={0.01} value={p} onChange={(e) => setP(Number(e.target.value))} className="w-full" />
        </div>
      </div>
      <Plotly
        ...
        layout={{
          ...
          plot_bgcolor: 'rgba(15,15,25,1)',
          font: { color: '#9ca3af' },
          xaxis: { title: { text: 'Node' }, gridcolor: '#1e1e2e' },
          yaxis: { title: { text: 'Node' }, gridcolor: '#1e1e2e' },
          margin: { t: 20, r: 20, b: 45, l: 55 },
        }}
        config={{ displayModeBar: false }}
        style={{ width: '100%', height: 360 }}
  ...
  return card(
    ...
    <>
      <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <div>
          <label className="text-sm text-gray-300">Steps: {steps}</label>
          <input type="range" min={1} max={8} step={1} value={steps} onChange={(e) => setSteps(Number(e.target.value))} className="w-full" />
        </div>
        <div>
          <label className="text-sm text-gray-300">Source node: {source}</label>
          <input type="range" min={0} max={Math.max(0, n - 1)} step={1} value={source} onChange={(e) => setSource(Number(e.target.value))} className="w-full" />
        </div>
      </div>
      <Plotly
        ...
        layout={{
          ...
          font: { color: '#9ca3af' },
          xaxis: { title: { text: 'Node index' }, gridcolor: '#1e1e2e' },
          yaxis: { title: { text: 'Signal after aggregation' }, gridcolor: '#1e1e2e' },
          margin: { t: 20, r: 20, b: 45, l: 55 },
        }}
        config={{ displayModeBar: false }}
        style={{ width: '100%', height: 360 }}
