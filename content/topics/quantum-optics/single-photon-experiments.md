...
For a Mach-Zehnder interferometer with a single photon input, compute the detection probabilities $P_c(\phi)$ and $P_d(\phi)$ as a function of the phase shift $\phi$. Then consider two single photons entering the first beam splitter from opposite ports. Compute the two-photon output state and show that, at the first beam splitter, the Hong-Ou-Mandel effect produces only the $|2,0\rangle$ and $|0,2\rangle$ terms. Propagate these through the phase shift and second beam splitter, and determine which coincidence outcomes survive. Compare the phase sensitivity of this two-photon scheme with the single-photon case.
