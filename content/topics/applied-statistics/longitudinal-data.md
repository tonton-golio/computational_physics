...
* Longitudinal data is grouped data with a time axis added — the random effects framework you already know handles it naturally, once you account for within-subject correlation and the ordering of observations.
* Missing data is never just a nuisance — the *mechanism* of missingness (MCAR, MAR, or MNAR) determines which analyses are valid and which silently bias your results.
* The choice of correlation structure (compound symmetry, AR(1), unstructured) is a modeling decision that should be informed by the data and evaluated with information criteria, not just picked for convenience.
* Repeated measures ANOVA is a special case of the linear mixed model — it requires complete data and equal spacing, while LMMs handle the messier reality that subjects drop out and miss appointments.

## What Comes Next

Throughout this topic you have built up a frequentist toolkit: distributions, likelihood, chi-square, hypothesis tests, ANOVA, experimental design, mixed models, and longitudinal analysis. All of it treats probability as a long-run frequency.

Bayesian statistics offers a complementary viewpoint: probability as a degree of belief. You start with what you knew before seeing the data, update it with the likelihood, and get a posterior distribution. The likelihood function you have been using since the PDFs section is the bridge — Bayesian and frequentist methods share the same engine, just pointed in different directions.

## Check Your Understanding

1. A patient drops out of a clinical trial because their condition worsens and they can no longer attend. Is this MCAR, MAR, or MNAR? What are the consequences for a linear mixed model analysis that uses all available data up to dropout?
2. You fit a longitudinal model and must choose between compound symmetry and AR(1) for the correlation structure. Describe in words what each structure assumes, and give an example of a real-world measurement process where each would be appropriate.
3. Repeated measures ANOVA gives a significant result but Mauchly's test rejects the sphericity assumption. What should you do, and what are the trade-offs between the Greenhouse-Geisser and Huynh-Feldt corrections?

## Challenge

You are analyzing five years of blood pressure data from 200 patients, measured every six months. By year 3, roughly 40 patients have dropped out. Describe the complete analysis strategy you would use: (a) how you would characterize the dropout mechanism and check which type of missingness is most plausible, (b) which model you would fit and why, (c) how you would choose the correlation structure, and (d) what you would do if you suspected the missing data was MNAR. Be specific about which tools from this lesson apply at each step.
